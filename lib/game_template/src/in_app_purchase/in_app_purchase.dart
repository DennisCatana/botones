...
class InAppPurchaseController extends ChangeNotifier {
  ...
  Future<void> buy() async {
    ...
    if (response.error != null) {
      _reportError('There was an error when making the purchase: ${response.error}');
      return;
    }
    ...
    if (response.productDetails.length != 1) {
      ...
      _reportError('There was an error when making the purchase: product ${AdRemovalPurchase.productId} does not exist?');
      return;
    }
    ...
    try {
      ...
      _log.severe('Problem with calling inAppPurchaseInstance.buyNonConsumable(): $e');
    }
  }
  ...
  Future<void> _listenToPurchaseUpdated(List<PurchaseDetails> purchaseDetailsList) async {
    for (final purchaseDetails in purchaseDetailsList) {
      _log.info(() => 'New PurchaseDetails instance received: productID=${purchaseDetails.productID}, status=${purchaseDetails.status}, purchaseID=${purchaseDetails.purchaseID}, error=${purchaseDetails.error}, pendingCompletePurchase=${purchaseDetails.pendingCompletePurchase}');

      if (purchaseDetails.productID != AdRemovalPurchase.productId) {
        _log.severe("The handling of the product with id '${purchaseDetails.productID}' is not implemented.");
